...
| `BLINKIFY_ADMIN_EMAILS` | No | Comma-separated emails for admin-only routes (e.g. `/admin`). Server-only; do not use `NEXT_PUBLIC_` so admin list is not exposed to the client. |
